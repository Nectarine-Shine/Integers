...
namespace Integers
{
    ...
    public class Integer
    {
        ...
        private static Integer Multiply(Integer a, Integer b)
        {
            ...
            for (var i = 0; i < a.Size; i++)//17
            {
                for (int j = 0, reduce = 0; (j < b.Size) || (reduce > 0); j++)//13
                {
        ...
        private static Integer Div(Integer a, Integer b)// 17 13
        {
            var result = Zero;
            var current = Zero;
            ...
            for (var i = a.Size - 1; i >= 0; i--)
            {
                ...
                while (left <= right)
                {
                    var middle = (left + right) / 2;// 2
                    var cur = b * Exp((byte) middle, i);// 26
                    if (cur <= current)
                    {
                        x = middle;
                        left = middle + 1;
        ...
        public static Integer ModPow(Integer value, Integer power, Integer module)//19 5 21
        {
            var binaryValue = ConvertToBinary(power);//101

            var arr = new Integer[binaryValue.Count];//19, 4, 16
            arr[0] = value;//19
            for (var i = 1; i < binaryValue.Count; i++)
                arr[i] = arr[i - 1] * arr[i - 1] % module;//4 16

            var multiplication = One;
            var zero = Zero;
            for (var j = 0; j < binaryValue.Count; j++)
                if (binaryValue[j] > zero)
                    multiplication *= binaryValue[j] * arr[j];//304

            return multiplication % module;//10
        }
